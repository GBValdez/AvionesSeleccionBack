...
namespace AvionesBackNet.utils
{
    public class Utils
    {
        ...
        public async static Task<errResPag<TDto>> paginate<T, TDto>(IQueryable<T> query, pagQueryDto data, IMapper mapper)
        {
            errResPag<TDto> res = new errResPag<TDto>();
            int total = await query.CountAsync();

            if (total == 0)
            {
                res.resPag = new resPag<TDto>
                {
                    items = new List<TDto>(),
                    total = 0,
                    index = 0,
